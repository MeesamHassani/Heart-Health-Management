...
def train_model(extracted_training_data, extracted_training_target):
    """Function to train our svm model"""
    svm_model = svm.SVC(C=1, kernel="linear")
    model =  svm_model.fit(extracted_training_data, extracted_training_target)
    return model
...
def user_data_prediction_by_model(model, array):
    print(" user prediction ", model.predict(array))
...
array = []
arr =  numpy.array([53,1,0,140,203,1,0,155,1,3.1,0,0,3])
arr = arr.astype(numpy.float)
array.append(arr)
print(array)
user_data_prediction_by_model(model, array)